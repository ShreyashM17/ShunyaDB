...
impl Engine {
    pub fn new(wal_path: &str) -> Self {
        ...
        for table in tables {
            let idx = HashIndex::load(&table).unwrap_or_else(|_| {
    ...
    pub fn get(&mut self, table: &str, filter: Filter) -> Result<Vec<Page>> {
        ...
        if let Filter::ByKeyValueEq(ref field, ref value) = filter {
            ...
            if !val_as_str.is_empty()
                && let Some(index) = self.index.get(table)
                && let Some(id_pairs) = index.lookup(field, &val_as_str)
            {
                index_record_ids = id_pairs.iter().map(|(_, rid)| *rid).collect();
                page_ids = id_pairs.iter().map(|(pid, _)| *pid).collect();
            }
        }
        if !index_record_ids.is_empty() && !page_ids.is_empty() {
            page_ids.dedup();
    ...
    pub fn update(
        ...
        for p in meta.pages.clone() {
            let file_path = util::page_file(table, p.id);
            let mut page =
                io::load_page_from_disk(&file_path).expect("Unable to load Page from Disk");
            ...
            self.cache
                .put(&format!("{}_page_{}", table, p.id), page.clone());
    ...
    pub fn delete(&mut self, table: &str, filter: Filter) -> Result<usize> {
        ...
        for p in &meta.pages {
            ...
            let ids_to_delete: Vec<u64> = page
                .records
                .iter()
                        ...
                        if let Some(pages) = buffer.get_mut(&entry.table) {
                            ...
                            if let Some(last_page) = pages.last_mut() {
                                if last_page.is_full() {
                                    let mut new_page =
                                        Page::new(last_page.id + 1, self.pagecapacity);
                                    new_page
                                        .insert(record)
                                        .expect("Insertion failed to new record while replay");
                                    pages.push(new_page);
                                } else {
                                    last_page
                                        .insert(record)
                                        .expect("Insertion failed to existing file while replay");
                                }
                            } else {
